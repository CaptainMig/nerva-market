...
// /api/market.js — v4.6: FMP stable URL + full error transparency
// ═══════════════════════════════════════════════════════════════
...
// FMP stable base URL (correct for all plans)
...
export default async function handler(req, res) {
  ...
  try {
    // Three parallel requests — all within Vercel 10s timeout
    const [quoteResp, spyResp, qqqResp] = await Promise.all([
      fetch(`${FMP}/quote/${ALL_SYMS.join(',')}?apikey=${KEY}`),
      fetch(`${FMP}/historical-price-full/SPY?timeseries=200&apikey=${KEY}`),
      fetch(`${FMP}/historical-price-full/QQQ?timeseries=200&apikey=${KEY}`),
    ]);

    // Always parse the quote response body for debugging
    const quoteText = await quoteResp.text();
    let quoteData;
    try { quoteData = JSON.parse(quoteText); } catch { quoteData = null; }

    // If not an array, FMP returned an error — expose it
    if (!Array.isArray(quoteData)) {
      return res.status(200).json({
        error: 'FMP returned non-array response',
        fmp_status: quoteResp.status,
        fmp_response: typeof quoteData === 'object' ? quoteData : quoteText.slice(0,300),
    ...
    // Parse history
    const spyHist = spyResp.ok ? parseHistory(await spyResp.json()) : {sma20:0,sma50:0,sma200:0,rsi:55};
    const qqqHist = qqqResp.ok ? parseHistory(await qqqResp.json()) : {sma20:0,sma50:0,sma200:0,rsi:55};
